...
namespace UIEditor.Entity.Control
{
    ...
    /// </summary>
    [Serializable]
    public class ColorLightNode : ControlBaseNode
    {
        private static int index = 0;

        public string ShortcutColor { get; set; }

        #region 构造函数
        public ColorLightNode()
        {
            index++;

            this.Text = "彩灯开关_" + index;
            this.ShortcutColor = "";

            Name = ImageKey = SelectedImageKey = MyConst.Controls.KnxColorLightType;
        }


        public ColorLightNode(KNXColorLight knx)
            : base(knx)
        {

            this.ShortcutColor = knx.ShortcutColor;

            Name = ImageKey = SelectedImageKey = MyConst.Controls.KnxColorLightType;

        }

        protected ColorLightNode(SerializationInfo info, StreamingContext context) : base(info, context) { }
        #endregion

        #region knx
        public KNXColorLight ToKnx()
        {
            var knx = new KNXColorLight();

            base.ToKnx(knx);

            knx.ShortcutColor = this.ShortcutColor;
        ...
        public override void DisplayProperties(Grid grid)
        {
            ...
            currentRow++;
            grid.Rows.Insert(currentRow);
            grid[currentRow, MyConst.NameColumn] = new SourceGrid.Cells.Cell("颜色名称");
            grid[currentRow, MyConst.NameColumn].View = nameModel;
            grid[currentRow, MyConst.ValueColumn] = new SourceGrid.Cells.Cell(this.ShortcutColor);
            grid[currentRow, MyConst.ValueColumn].AddController(valueChangedController);
            grid[currentRow, MyConst.ButtonColumn] = new SourceGrid.Cells.Button("...");
        ...
        public override void ChangePropValues(CellContext context)
        {
            ...
            switch (row)
            {
                ...
                #region 扩展属性
                case 13:
                    this.ShortcutColor = context.Value.ToString();
                    break;
                default:
                    ShowSaveEntityMsg(MyConst.Controls.KnxColorLightType);
                    break;
                #endregion
            }
