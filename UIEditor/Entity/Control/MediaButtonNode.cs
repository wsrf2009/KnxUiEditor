...
namespace UIEditor.Entity.Control
{
    ...
    public class MediaButtonNode : ControlBaseNode
    {
        ...
        public MediaButtonType MediaButtonType { get; set; }
        ...
        public MediaButtonNode()
        {
            ...
            this.CustomImage = MyConst.DefaultIcon;
            this.MediaButtonType = MediaButtonType.Play;

            this.Text = "多媒体开关_" + index;
            Name = ImageKey = SelectedImageKey = MyConst.Controls.KnxMediaButtonType;
        }

        public MediaButtonNode(KNXMediaButton knx)
            : base(knx)
        {
            this.CustomImage = knx.CustomImage;
            if (string.IsNullOrEmpty(knx.MediaButtonType))
            {
                this.MediaButtonType = MediaButtonType.Play;
            }
            else
            {
                this.MediaButtonType = (MediaButtonType)Enum.Parse(typeof(MediaButtonType), knx.MediaButtonType);
            }
        ...
        protected MediaButtonNode(SerializationInfo info, StreamingContext context) : base(info, context) { }
        #endregion

        #region knx
        public KNXMediaButton ToKnx()
        {
            var knx = new KNXMediaButton();
            base.ToKnx(knx);

            knx.CustomImage = this.CustomImage;
            knx.MediaButtonType = this.MediaButtonType.ToString();
        ...
        public override void DisplayProperties(Grid grid)
        {
            ...
            currentRow++;
            grid.Rows.Insert(currentRow);
            grid[currentRow, MyConst.NameColumn] = new SourceGrid.Cells.Cell("自定义图片");
            grid[currentRow, MyConst.NameColumn].View = nameModel;
            ...
            {
                grid[currentRow, MyConst.ValueColumn] = new SourceGrid.Cells.Cell(this.CustomImage);
                grid[currentRow, MyConst.ValueColumn].Image =
                    ImageHelper.LoadImage(Path.Combine(MyCache.ProjImagePath, this.CustomImage));
            }
            grid[currentRow, MyConst.ValueColumn].AddController(valueChangedController);
            grid[currentRow, MyConst.ButtonColumn] = new SourceGrid.Cells.Button("...");
            ...
            currentRow++;
            grid.Rows.Insert(currentRow);
            grid[currentRow, MyConst.NameColumn] = new SourceGrid.Cells.Cell("自定义类型");
            grid[currentRow, MyConst.NameColumn].View = nameModel;
            grid[currentRow, MyConst.ValueColumn] = new SourceGrid.Cells.Cell(this.MediaButtonType,
                typeof(MediaButtonType));
            grid[currentRow, MyConst.ValueColumn].AddController(valueChangedController);
        ...
        public override void ChangePropValues(CellContext context)
        {
            ...
            switch (row)
            {
                ...
                #region 基础属性
                case 13:
                    this.CustomImage = context.Value.ToString();
                    break;
                case 14:
                    this.MediaButtonType = (MediaButtonType)context.Value;
                    break;
                default:
                    ShowSaveEntityMsg(MyConst.Controls.KnxMediaButtonType);
                    break;
                #endregion
            }
        }
